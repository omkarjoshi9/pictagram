...
export default function Create() {
  ...
  const uploadImage = async (imageFile: File) => {
    ...
    try {
      ...
      // Upload the file
      const response = await axios.post('/api/posts/image', formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
      
      // Store the image URL returned from the server
      ...
      toast({
        title: 'Error',
        description: 'Failed to upload image. Please try a smaller file or different format.',
        variant: 'destructive'
      });
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!uploadedImageUrl) {
      toast({
        ...
        description: "Please wait for the image to upload completely.",
        variant: "destructive"
      });
      setIsSubmitting(false);
      return;
    ...
    try {
      // Create post data with the URL from file upload
      const postData = {
        userId: user.id,
        imageUrl: uploadedImageUrl, // Use the URL from the file upload instead of base64 string
        caption,
        feeling,
        categories
      };
              ...
              <div className="space-y-6">
                      ...
                      {previewURL ? (
                        ...
                        <div className="h-full w-full flex flex-col items-center justify-center pt-7">
                          ...
                          <p className="pt-1 text-sm tracking-wider text-muted">
                            {selectedImageName || "Select a photo"}
                          </p>
                        </div>
                      )}
                      <input 
                ...
                <div className="pt-4">
                  <button 
                    ...
                    disabled={isSubmitting || !uploadedImageUrl || !caption || isUploading}
                  >
                    {isSubmitting ? (
                      <>
